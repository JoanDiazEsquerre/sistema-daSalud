...
public class ReportGenDaoImpl extends HibernateDaoSupport implements ReportGenDao {
	...
	@Override // Necesario dos métodos porque para los casos que no se usa la conexion el
				// reporte resulta blanco.
	public void exportByFormatNotConnectDb(JRDataSource dt, String path, String type, Map parameters,
			String outputFileName, boolean defineAncho, Integer ancho, boolean defineLargo, Integer largo) {
		try {
			FacesContext context = FacesContext.getCurrentInstance();
			HttpServletResponse response = (HttpServletResponse) context.getExternalContext().getResponse();
	...
	@Override // Necesario dos métodos porque para los casos que no se usa la conexion el
				// reporte resulta blanco.
	public void exportByFormatNotConnectDb(JRDataSource dt, String path, String type, Map parameters,
			String outputFileName, String pathOut) {
		try {
			FacesContext context = FacesContext.getCurrentInstance();
			HttpServletResponse response = (HttpServletResponse) context.getExternalContext().getResponse();
			ServletContext scontext = (ServletContext) context.getExternalContext().getContext();
			...
			try {
				...
				if ("pdf".equals(type)) {
					...
					exporter.setParameter(JRPdfExporterParameter.JASPER_PRINT, jasperPrint);
					exporter.setParameter(JRPdfExporterParameter.OUTPUT_STREAM, arrayOutputStream);
					// exporter.setParameter(JRPdfExporterParameter.OUTPUT_FILE, pathOut +
					// outputFileName + ".pdf");
					exporter.exportReport();
					JasperExportManager.exportReportToPdfFile(jasperPrint,
							scontext.getRealPath(pathOut + outputFileName + ".pdf"));
					// cambia esto
				} else if ("xls".equals(type)) {
					response.setContentType("application/vnd.ms-excel");
					response.setHeader("Content-disposition", "attachment;filename=" + outputFileName + ".xls");
	...
	public void formatConConexionDB(String path, String type, Map parameters, String outputFileName) {
		 ...
		 * context.getExternalContext().getResponse(); //InputStream inputStream =
		 * context.getExternalContext().getResourceAsStream(path); //InputStream
		 * inputStream =
		 * JRLoader.getFileInputStream(context.getExternalContext().getRealPath(path));
		 * InputStream inputStream =
		 * JRLoader.getFileInputStream(context.getExternalContext().getRealPath(path));
		 * 
		 * ByteArrayOutputStream arrayOutputStream = new ByteArrayOutputStream(); byte[]
		 * bytes = null;
		 * 
		 * if (inputStream == null) { logger.error("NO CARGA ARCHIVO .JASPER");
		 * System.exit(2); }
		 * 
		 * try {
		 * 
		 ...
		 * 
		 * // upload the report JasperReport report = (JasperReport)
		 * JRLoader.loadObject(inputStream); JasperPrint jasperPrint = null;
		 * SessionFactoryImplementor impl = (SessionFactoryImplementor)
		 * getSessionFactory(); ConnectionProvider cp = impl.getConnectionProvider();
		 * Connection conn = cp.getConnection(); jasperPrint =
		 * JasperFillManager.fillReport(report, parameters, conn);
		 * 
		 * JRExporter exporter = null; if ("pdf".equals(type)) {
		 * response.setContentType("application/pdf");
		 * response.setHeader("Content-disposition", "attachment;filename=" +
		 * outputFileName + ".pdf"); exporter = new JRPdfExporter();
		 * exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
		 * exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, arrayOutputStream);
		 * } else if ("xls".equals(type)) {
		 * response.setContentType("application/vnd.ms-excel");
		 * response.setHeader("Content-disposition", "attachment;filename=" +
		 * outputFileName + ".xls"); exporter = new JRXlsExporter();
		 * exporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
		 * exporter.setParameter(JRExporterParameter.OUTPUT_STREAM, arrayOutputStream);
		 ...
		 * "public"); exporter.exportReport(); bytes = arrayOutputStream.toByteArray();
		 * response.setContentLength(bytes.length);
		 * response.getOutputStream().write(bytes, 0, bytes.length);
		 * 
		 * } catch (Exception e) { logger.error(e); } } } catch (JRException e) { //
		 * display stack trace in the browser logger.error(e); } catch
		 * (HibernateException e) { logger.error(e); throw e; } finally {
		 * getSessionFactory().getCurrentSession().disconnect();
		 * response.getOutputStream().flush(); response.getOutputStream().close();
		 * context.responseComplete(); } } catch (Exception j) { logger.error(j); }
		 */
	}

	/*
	 * public String printJasperNotConnectDb (JRDataSource dt,String path, Map
	 * parameters, String namePrinter){
	 * 
	 * String mensaje=""; try { FacesContext context =
	 * FacesContext.getCurrentInstance(); ServletContext scontext = (ServletContext)
	 * context.getExternalContext().getContext();
	 * 
	 * try { JasperPrint jasperPrint =
	 * JasperFillManager.fillReport(scontext.getRealPath(path), parameters, dt);
	 * JRExporter jrExporter = null ; try { PrinterName printerName =new
	 * PrinterName(namePrinter, null); PrintServiceAttributeSet
	 * printServiceAttributeSet = new HashPrintServiceAttributeSet();
	 * printServiceAttributeSet.add(printerName); //PrinterJob job =
	 * PrinterJob.getPrinterJob(); //PrintService service =
	 * PrintServiceLookup.lookupDefaultPrintService(); //
	 * job.setPrintService(service); //PrintRequestAttributeSet
	 * printRequestAttributeSet = new HashPrintRequestAttributeSet();
	 * //MediaSizeName mediaSizeName = MediaSize.ISO.A4.getMediaSizeName();
	 * //printRequestAttributeSet.add(mediaSizeName);
	 * //printRequestAttributeSet.add(new Copies(1)); if( !namePrinter.equals("")) {
	 * jrExporter =new JRPrintServiceExporter();
	 * jrExporter.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
	 * //jrExporter.setParameter(JRPrintServiceExporterParameter.PRINT_SERVICE,
	 * service); //jrExporter.setParameter(JRPrintServiceExporterParameter.
	 * PRINT_REQUEST_ATTRIBUTE_SET, printRequestAttributeSet);
	 * jrExporter.setParameter(JRPrintServiceExporterParameter.
	 * PRINT_SERVICE_ATTRIBUTE_SET, printServiceAttributeSet);
	 ...
	 * Boolean.FALSE); jrExporter.exportReport(); mensaje="Impreso Correctamente"; }
	 * else {
	 * 
	 * System.err.println("Impresora " + " no encontrada"); } } catch(Exception e) {
	 * e.printStackTrace(); mensaje="Impresora No Encontrada"; } } catch
	 * (JRException e) { logger.error(e); } catch (HibernateException e) {
	 * logger.error(e); throw e; } finally { }
	 * 
	 * } catch (Exception j) { logger.error(j); }
	 * 
	 * return mensaje;
	 * 
	 * }
	...
	public String printJasperNotConnectDb(JRDataSource dt, String path, Map parameters, String enlacePrint,
			String namePrint, Integer nroCopias) {
			...
			try {
				...
				try {
					...
					if (!enlacePrint.equals("")) {
						...
						jrExporter.setParameter(JRPrintServiceExporterParameter.PRINT_REQUEST_ATTRIBUTE_SET,
								printRequestAttributeSet);
						jrExporter.setParameter(JRPrintServiceExporterParameter.DISPLAY_PAGE_DIALOG, Boolean.FALSE); // who
																														// doesn't
																														// like
																														// self-explanatory
																														// attributes!
